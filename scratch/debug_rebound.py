...
print(f"Total duration: {traj[-1][0]:.3f}s")
z_vals = [s[3] for s in traj]
print(f"Z max: {max(z_vals)*100:.1f} cm, Z min: {min(z_vals)*100:.2f} cm")

# Find local maxima after first impact (rebounds)
impact_time = None
for i in range(1, len(traj)-1):
    t, x, y, z = traj[i][:4]
    z_prev = traj[i-1][3]
    z_next = traj[i+1][3]
    if z_prev > z and z_next >= z and impact_time is None:
        impact_time = t
        print(f"First impact at t={t:.4f}s, z={z*100:.2f} cm")
    elif impact_time is not None and z > z_prev and z >= z_next and (z - min(z_vals)) > 0.001:
        rebound_h = (z - min(z_vals)) * 100
        print(f"Rebound peak at t={t:.4f}s: height = {rebound_h:.2f} cm above floor")

print("\nAll impacts recorded in drop result:")
