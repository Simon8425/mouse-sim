...
export function createOverlayLayer(
  ...
  function createLabelSprite(text: string, planeRadius: number): THREE.Sprite {
    ...
    sprite.scale.set(planeRadius * 0.35, planeRadius * 0.0875, 1);
    return sprite;
  }

  return {
    apply(spec: OverlaySpec | null) {
      ...
      const r = options.planeRadius || 0.1;

      // Load Vector
      if (spec.loadVector) {
        const dir = new THREE.Vector3(...spec.loadVector.direction).normalize();
      ...
      // Contact Plane
      if (spec.contactPlane) {
        const normal = new THREE.Vector3(...spec.contactPlane.normal).normalize();
