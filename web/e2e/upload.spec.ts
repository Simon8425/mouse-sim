...
import { modelLoaded, fixturePath, collectPageErrors, expectNoConsoleErrors } from './helpers';
...
test.describe('upload flow', () => {
  ...
  test('STEP upload displays diagnostic error', async () => {
    await modelLoaded(page);
    await page.getByRole('button', { name: 'Upload geometry' }).click();
    const fileInput = page.locator('input[type="file"]');
    await fileInput.setInputFiles(path.resolve(__dirname, 'fixtures', 'unsupported.step'));
    await expect(page.locator('.dropzone-error')).toContainText(/unsupported|STEP/i, { timeout: 10_000 });
    await expectNoConsoleErrors(page, errors);
  });
